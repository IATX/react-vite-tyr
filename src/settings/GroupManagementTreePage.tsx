...
/** Payload handed to the action callbacks (add / edit / delete). */
...
interface GroupManagementTreeProps {
  addGroup: (node: GroupNodePayload) => void;
  editGroup: (node: GroupNodePayload) => void;
  deleteGroup: (node: GroupNodePayload) => void;
}
...
const GroupManagementTree = forwardRef<GroupTreeRef, GroupManagementTreeProps>(({ addGroup, editGroup, deleteGroup }, ref) => {
  ...
  const buildPayload = (node: DataNode): GroupNodePayload => {
  ...
  const titleRender: TreeProps<DataNode>['titleRender'] = (node) => {
    ...
    const runAction = (
      e: React.MouseEvent,
      action: (payload: GroupNodePayload) => void,
    ) => {
      e.stopPropagation();
        ...
        <span className="tyr-tree-actions" onClick={(e) => e.stopPropagation()}>
          <Tooltip title="Add sub-group">
            <span
              ...
              onClick={(e) => runAction(e, addGroup)}
            >
              <PlusOutlined />
            </span>
          </Tooltip>
          {!isRoot && (
            <Tooltip title="Edit">
              <span
                className="tyr-tree-action"
                onClick={(e) => runAction(e, editGroup)}
              >
                <EditOutlined />
              </span>
            </Tooltip>
          ...
          {canDelete && (
            <Tooltip title="Delete">
              <span
                ...
                onClick={(e) => runAction(e, deleteGroup)}
              >
                <DeleteOutlined />
              </span>
            </Tooltip>
