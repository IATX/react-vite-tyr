...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			...
    			{
		            id: 'ungwvpfr', label: '结算单状态', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkLgtnnfgg' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkLgtnnfgg' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_fzwlye', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
                ...
                elem: Parameterization('ViewTbLgtnnfggBdonpj', {
                    ...
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '电费结算单列表',
                            subheader: '电费结算单查阅列表',
                            elem: Parameterization('QueryFzwlye', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbLgtnnfggBdonpj', {
                state: {
                    key: 'ViewTbLgtnnfggBdonpj',
                    initialData: {
                       'pkLgtnnfgg': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryFzwlye', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="电费结算单" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={crurpjzHandler}
	                                aria-describedby={'crurpjz'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
