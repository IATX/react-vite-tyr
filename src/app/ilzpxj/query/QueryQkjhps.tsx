...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'yqxtneyb', label: '开始日期', minWidth: 150, align: 'center', type: 'string', truncate: true
    			...
    			{
		            id: 'ephegbar', label: '截至日期', minWidth: 150, align: 'center', type: 'string', truncate: true
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const ts = String(val).length === 10 ? val * 1000 : val;
                    	const date = new Date(ts);
    			...
    			{
		            id: 'btcqaupc', label: '尖峰时段', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'lzbffroj', label: '高峰时段', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'yobavxkc', label: '平时段', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'jmootdaz', label: '低谷时段', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'effhvodl', label: '生成日期', minWidth: 150, align: 'center', type: 'string', truncate: true
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const ts = String(val).length === 10 ? val * 1000 : val;
                    	const date = new Date(ts);
		            ...
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkNiavzhpt' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkNiavzhpt' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		yqxtneyb: props.queryParams?.yqxtneyb ?? '',
    		ephegbar: props.queryParams?.ephegbar ?? '',
    		effhvodl: props.queryParams?.effhvodl ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_qkjhps', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '国网上海市电力公司电费账单合计详情表单',
                subheader: '国网上海市电力公司电费账单合计详情表单',
                elem: Parameterization('ViewTbNiavzhptBhzzpz', {
                    key: 'ViewTbNiavzhptBhzzpz',
                    initialData: {
                       'pkNiavzhpt': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '上电电费账单示数合计列表',
                            subheader: '上电电费账单示数合计列表',
                            elem: Parameterization('QueryQkjhps', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbNiavzhptBhzzpz', {
                state: {
                    key: 'ViewTbNiavzhptBhzzpz',
                    initialData: {
                       'pkNiavzhpt': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryQkjhps', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkNiavzhpt', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_niavzhpt_bhzzpz', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="电费示数合计详情表单" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={fjokwlhHandler}
	                                aria-describedby={'fjokwlh'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
