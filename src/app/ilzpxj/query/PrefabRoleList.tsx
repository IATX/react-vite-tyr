...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const [pkVal, setPkVal] = useState<any>(null);
    const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
    const [aljlpierOpen, setAljlpierOpen] = useState(false);
    ...
    const columnData: ColumnDataProp[] = [
    			...
    			{
		            id: 'rolekey', label: 'Key', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'memo', label: 'Description', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'purviewcontent', label: 'Permissions', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'autosp', label: 'Editor', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    setPkVal(rowData['id' as keyof Data]);
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		rname: props.queryParams?.rname ?? '',
    		rkey: props.queryParams?.rkey ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/prefab_role_list', params, {
            headers: {
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('rid', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/prefab_role_add', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
    ...
    const ljlpierHandler = () => {
        	setAljlpierOpen(true);
    }
    ...
    return (
        <>
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="Role Information" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={ljlpierHandler}
	                                aria-describedby={'ljlpier'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
            ...
            <EditDataDialog
                title="Role detail view"
                open={isEditDialogOpen}
                onClose={() => {
                    setIsEditDialogOpen(false);
                }}
                children={WrapSoloFormNode(Parameterization('PrefabRoleEdit', {
                    key: 'PrefabRoleEdit',
                    initialData: {
                        'rid': pkVal
                    },
