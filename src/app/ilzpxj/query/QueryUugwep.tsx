...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'imxdflym', label: '户号', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'xdsjflqz', label: '商户', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            ...
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkWzghpmog' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkWzghpmog' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		imxdflym: props.queryParams?.imxdflym ?? '',
    		xdsjflqz: props.queryParams?.xdsjflqz ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_uugwep', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '商户电价设置表单',
                subheader: '商户电价设置表单',
                elem: Parameterization('ViewTbWzghpmogSemlgb', {
                    key: 'ViewTbWzghpmogSemlgb',
                    initialData: {
                       'pkWzghpmog': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '电价设置列表',
                            subheader: '商户电价维护列表',
                            elem: Parameterization('QueryUugwep', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbWzghpmogSemlgb', {
                state: {
                    key: 'ViewTbWzghpmogSemlgb',
                    initialData: {
                       'pkWzghpmog': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryUugwep', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkWzghpmog', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_wzghpmog_semlgb', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="电价设置" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={vqrvuewHandler}
	                                aria-describedby={'vqrvuew'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
