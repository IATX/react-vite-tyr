...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'cdtylzeq', label: '执行时间从', minWidth: 150, align: 'center', type: 'string', truncate: true
    			...
    			{
		            id: 'dmnbozws', label: '执行时间至', minWidth: 150, align: 'center', type: 'string', truncate: true
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const ts = String(val).length === 10 ? val * 1000 : val;
                    	const date = new Date(ts);
    			...
    			{
		            id: 'uxmevhzr', label: '用电分类', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'tgfmfgug', label: '用电制度', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'vjtwicby', label: '电压等级', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'yqjhrqpv', label: '代理购电价格', minWidth: 150, align: 'center', type: 'number', truncate: true
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const moneyVal = new Intl.NumberFormat(
					        undefined,
					        {
					            style: 'currency',
					            currency: VITE_JET_CURRENCY_CODE, 
                    	...
                    	return <><Tooltip title={val} arrow><span>{moneyVal}</span></Tooltip></>
		            }
				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkXntlcwoh' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkXntlcwoh' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		uxmevhzr: props.queryParams?.uxmevhzr ?? '',
    		tgfmfgug: props.queryParams?.tgfmfgug ?? '',
    		vjtwicby: props.queryParams?.vjtwicby ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_qrogsu', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '代理购电工商业用户电价表（普通和1.5倍）',
                subheader: '代理购电工商业用户电价表（普通和1.5倍）',
                elem: Parameterization('ViewTbXntlcwohUzlwbg', {
                    key: 'ViewTbXntlcwohUzlwbg',
                    initialData: {
                       'pkXntlcwoh': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '代理购电价格一五列表',
                            subheader: '代理购电价格一五倍维护列表',
                            elem: Parameterization('QueryQrogsu', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbXntlcwohUzlwbg', {
                state: {
                    key: 'ViewTbXntlcwohUzlwbg',
                    initialData: {
                       'pkXntlcwoh': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryQrogsu', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_xntlcwoh_uzlwbg', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="代理购电工商业用户电价表" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={fewfofkHandler}
	                                aria-describedby={'fewfofk'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
