...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'jchzjhug', label: '账单周期（从）', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
    			...
    			{
		            id: 'dnsmnqeh', label: '账单周期（至）', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const ts = String(val).length === 10 ? val * 1000 : val;
                    	const date = new Date(ts);
    			...
    			{
		            id: 'ythuiqfx', label: '发电户号', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'knhbvtpp', label: '户名', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'okmaqgil', label: '发电方式', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'irncwabt', label: '发电量', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            				},	
    			{
		            id: 'okfhbplj', label: '上网电量', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            				},	
    			{
		            id: 'nitcgncr', label: '结算金额', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkPiclbkqk' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkPiclbkqk' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		ythuiqfx: props.queryParams?.ythuiqfx ?? '',
    		knhbvtpp: props.queryParams?.knhbvtpp ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_xwztav', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                ...
                elem: Parameterization('ViewTbPiclbkqkIsjbmb', {
                    ...
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '国网上海市电力公司电费账单列表',
                            subheader: '一般工商业用电账单列表',
                            elem: Parameterization('QueryXwztav', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbPiclbkqkIsjbmb', {
                state: {
                    key: 'ViewTbPiclbkqkIsjbmb',
                    initialData: {
                       'pkPiclbkqk': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryXwztav', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_piclbkqk_isjbmb', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="国网上海市电力公司电费账单" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={hrxoknmHandler}
	                                aria-describedby={'hrxoknm'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
