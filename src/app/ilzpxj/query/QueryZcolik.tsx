...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			...
    			{
		            id: 'armiyjlh', label: '联系人', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'eznomrac', label: '联系电话', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkXbbyezwt' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkXbbyezwt' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_zcolik', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '商户基本信息表单',
                subheader: '商户基本信息表单',
                elem: Parameterization('ViewTbXbbyezwtKxchdy', {
                    key: 'ViewTbXbbyezwtKxchdy',
                    initialData: {
                       'pkXbbyezwt': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '商户列表',
                            subheader: '商户信息管理列表',
                            elem: Parameterization('QueryZcolik', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbXbbyezwtKxchdy', {
                state: {
                    key: 'ViewTbXbbyezwtKxchdy',
                    initialData: {
                       'pkXbbyezwt': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryZcolik', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_xbbyezwt_kxchdy', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="商户信息表单" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={xnuprmbHandler}
	                                aria-describedby={'xnuprmb'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
