...
import { useBillingActions } from '../js/query_gxffwv';
...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, { page?: number, pageSize?: number, queryParams?: any }> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
        {
            id: 'colVcuumj', label: '商户号', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'yagcdbmm', label: '商户名称', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'xhxzrkaw', label: '用电类别', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'odjdvwac', label: '结算月份', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        ...
        {
            id: 'ungwvpfr', label: '状态', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'columnCsdv', label: '操作', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
                return (<>
                    {rowData.acadkryr === '0' && (
                        <Button size='small' onClick={() => {
                            reviewSettlementStatement(rowData.pkLgtnnfgg, page, pageSize, queryData);
                        }}
                            sx={{
                                textTransform: 'none',
                                fontSize: '0.875rem',
                    ...
                    {rowData.acadkryr === '100' && (
                        <Button size='small' onClick={() => {
                            viewSettlementStatement(rowData.pkLgtnnfgg, page, pageSize, queryData);
                        }}
                            sx={{
                                textTransform: 'none',
                                fontSize: '0.875rem',
                    ...
                    <Button size='small' color="error" onClick={() => {
                        deleteSettlementStatement(rowData.pkLgtnnfgg, refreshTable);
                    }}
                        sx={{
                            textTransform: 'none',
                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
        colVcuumj: props.queryParams?.colVcuumj ?? '',
        yagcdbmm: props.queryParams?.yagcdbmm ?? '',
        odjdvwac: props.queryParams?.odjdvwac ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_gxffwv', params, {
            headers: {
