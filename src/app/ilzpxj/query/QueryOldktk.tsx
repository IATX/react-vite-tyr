...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'zonfitbq', label: '开始时间', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
    			...
    			{
		            id: 'fauaqxnr', label: '截至时间', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const ts = String(val).length === 10 ? val * 1000 : val;
                    	const date = new Date(ts);
    			...
    			{
		            id: 'bpyigirl', label: '序号', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            				},	
    			{
		            id: 'wdqoejrk', label: '名称', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'cuiwtgyv', label: '明细', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'vasfizlg', label: '计算关系', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'zvhavftu', label: '数值', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            ...
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkIfsxuxei' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkIfsxuxei' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		wdqoejrk: props.queryParams?.wdqoejrk ?? '',
    		cuiwtgyv: props.queryParams?.cuiwtgyv ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_oldktk', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '代理购电价格表',
                subheader: '代理购电价格表',
                elem: Parameterization('ViewTbIfsxuxeiLiyiee', {
                    key: 'ViewTbIfsxuxeiLiyiee',
                    initialData: {
                       'pkIfsxuxei': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '代理购电价格普通列表',
                            subheader: '代理购电价格普通列表',
                            elem: Parameterization('QueryOldktk', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbIfsxuxeiLiyiee', {
                state: {
                    key: 'ViewTbIfsxuxeiLiyiee',
                    initialData: {
                       'pkIfsxuxei': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryOldktk', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkIfsxuxei', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_ifsxuxei_liyiee', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="代理购电价格表" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={wwiocjbHandler}
	                                aria-describedby={'wwiocjb'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
