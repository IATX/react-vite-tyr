...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'cdtylzeq', label: '开始时间', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
    			...
    			{
		            id: 'dmnbozws', label: '截至时间', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            , render: (val: any, rowData: Data, allData: Data[]) => {
		            	const ts = String(val).length === 10 ? val * 1000 : val;
                    	const date = new Date(ts);
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_wxkson', params, {
            headers: {
		...
		if (isInMainArea) {
            setCurrentBayContent({
                ...
                elem: Parameterization('ViewTbXntlcwohUzlwbg', {
                    ...
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '代理购电价格列表',
                            subheader: '代理购电价格管理列表',
                            elem: Parameterization('QueryWxkson', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbXntlcwohUzlwbg', {
                state: {
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryWxkson', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
                        ...
                        <Box>
                        	<Tooltip title="代理购电工商业用户电价表" arrow>
	                            <Button
	                                ...
	                                onClick={ysjybckHandler}
	                                aria-describedby={'ysjybck'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
