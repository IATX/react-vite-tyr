...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'juxkhfnm', label: '发电户号', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'dvuiumlm', label: '上网电量kWh', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'ugngprmw', label: '发电单位', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'naqjbeoc', label: '总发电量kWh', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'iqxzxqxx', label: '账单日期', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'cwthkoid', label: '关联用电户号', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'thdvlhoj', label: '关联用电户用电量', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkTcbogtyo' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkTcbogtyo' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		juxkhfnm: props.queryParams?.juxkhfnm ?? '',
    		ugngprmw: props.queryParams?.ugngprmw ?? '',
    		iqxzxqxx: props.queryParams?.iqxzxqxx ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_ijjudr', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '光伏发电月度账单',
                subheader: '光伏发电月度账单',
                elem: Parameterization('ViewTbTcbogtyoAfmnqw', {
                    key: 'ViewTbTcbogtyoAfmnqw',
                    initialData: {
                       'pkTcbogtyo': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '发电月度账单列表',
                            subheader: '发电月度账单列表',
                            elem: Parameterization('QueryIjjudr', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbTcbogtyoAfmnqw', {
                state: {
                    key: 'ViewTbTcbogtyoAfmnqw',
                    initialData: {
                       'pkTcbogtyo': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryIjjudr', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkTcbogtyo', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_tcbogtyo_afmnqw', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="发电月度账单" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={dldphobHandler}
	                                aria-describedby={'dldphob'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
