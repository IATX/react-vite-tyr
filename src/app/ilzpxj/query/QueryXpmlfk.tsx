...
import { useBillingActions } from '../js/query_xpmlfk';
...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			...
    			{
		            id: 'columnPcol', label: '操作', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true , render: (val: any, rowData: Data, allData: Data[]) => {
                                     return (<>
                                            <a href="#" className='text-blue-600' onClick={() => {
                                                viewAddUpBillingEnergy(rowData.pkPiclbkqk, page, pageSize, queryData);
                                            }}>查阅</a>
                                        </>);
                                }
		            				},	
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_xpmlfk', params, {
            headers: {
