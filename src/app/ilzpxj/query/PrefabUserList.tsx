...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const [ahknqzutOpen, setAhknqzutOpen] = useState(false);

    const columnData: ColumnDataProp[] = [
    			{
		            id: 'name', label: 'Name', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'logid', label: 'Account', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'job', label: '用户职位', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'email', label: 'Email', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'phone', label: 'Phone Number', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'deptName', label: 'Dept./Roles', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'roleName', label: 'Roles', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'autosp', label: 'OP', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		uname: props.queryParams?.uname ?? '',
    		logid: props.queryParams?.logid ?? '',
    		uphone: props.queryParams?.uphone ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/prefab_user_list', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: 'User Information View',
                subheader: 'User Information View',
                elem: Parameterization('PrefabUserEdit', {
                    key: 'PrefabUserEdit',
                    initialData: {
                       'uid': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            ...
                            subheader: 'User Information Management List',
                            elem: Parameterization('PrefabUserList', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/PrefabUserEdit', {
                state: {
                    key: 'PrefabUserEdit',
                    initialData: {
                       'uid': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/PrefabUserList', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/prefab_user_add', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
    ...
    const hknqzutHandler = () => {
        	setAhknqzutOpen(true);
    }
                        ...
                        <Box>
                        	<Tooltip title="User Details" arrow>
	                            <Button
	                                ...
	                                onClick={hknqzutHandler}
	                                aria-describedby={'hknqzut'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
