...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'pulfqxdt', label: '账单年月', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
    			...
    			{
		            id: 'btvttbdp', label: '户号', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'snebfkdc', label: '户名', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'fpojjtxx', label: '用电类别', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'xwizrffi', label: '供电服务单位', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true 
		            				},	
    			{
		            id: 'jgeyksaw', label: '本期电量', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            				},	
    			{
		            id: 'fmjxqxwc', label: '本期电费', minWidth: 150, align: 'center', type: 'number', truncate: true, display: true 
		            				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkJkywwxtl' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkJkywwxtl' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		pulfqxdt: props.queryParams?.pulfqxdt ?? '',
    		snebfkdc: props.queryParams?.snebfkdc ?? '',
    		btvttbdp: props.queryParams?.btvttbdp ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_kaeany', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
                ...
                elem: Parameterization('ViewTbJkywwxtlMsurqp', {
                    ...
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            ...
                            subheader: '大工业用电账单列表',
                            elem: Parameterization('QueryKaeany', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbJkywwxtlMsurqp', {
                state: {
                    key: 'ViewTbJkywwxtlMsurqp',
                    initialData: {
                       'pkJkywwxtl': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryKaeany', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkJkywwxtl', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_jkywwxtl_msurqp', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
                        ...
                        <Box>
                        	<Tooltip title="国网上海市电力公司电费账单" arrow>
	                            <Button
	                                ...
	                                onClick={kwkamnvHandler}
	                                aria-describedby={'kwkamnv'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
