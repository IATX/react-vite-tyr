...
import { useBillingActions } from '../js/query_jokhkp';
...
import AddUpBillDialog from '../../../components/FormDialogSoloPage';
...
const DataTable: React.ForwardRefRenderFunction<DataTableRef, { page?: number, pageSize?: number, queryParams?: any }> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
        {
            id: 'jchzjhug', label: '账单周期从', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        ...
        {
            id: 'dnsmnqeh', label: '账单周期至', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
            , render: (val: any, rowData: Data, allData: Data[]) => {
                const ts = String(val).length === 10 ? val * 1000 : val;
                const date = new Date(ts);
        ...
        {
            id: 'ythuiqfx', label: '发电户号', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
                if (rowData.colRvhidn > 0) {
                    return (<>
                        <a href="#" className='text-blue-600' onClick={() => {
                            viewAddUpBillingEnergy(rowData.pkPiclbkqk);
                        }}>{val}</a>
                    </>);
                } else {
                    return (<>{val}</>)
        ...
        {
            id: 'knhbvtpp', label: '户名', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'juwpzays', label: '服务单位', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'iuqbiaih', label: '发电地址', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'colRvhidn', label: '状态码', minWidth: 150, align: 'center', type: 'number', truncate: true
        },
        {
            id: 'colObksuo', label: '账单状态', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true
        },
        {
            id: 'columnOnfg', label: '操作', minWidth: 150, align: 'center', type: 'string', truncate: true, display: true, render: (val: any, rowData: Data, allData: Data[]) => {
                if (rowData.colRvhidn === 0) {
                    return (
                        <Button
                            size='small'
                            onClick={() => {
                                addUpBillingEnergy(rowData.pkPiclbkqk, refreshTable);
                            }}
                            sx={{
                                textTransform: 'none',
                                fontSize: '0.875rem',
                ...
                } else if (rowData.colRvhidn === 1) {
                    return (
                        <Button
                            size='small'
                            onClick={() => {
                                submitForReviewBillingEnergy(rowData.pkPiclbkqk, refreshTable);
                            }}
                            sx={{
                                textTransform: 'none',
                                fontSize: '0.875rem',
                ...
                } else if (rowData.colRvhidn === 100) {
                    return (
                        <Button
                            size='small'
                            onClick={() => {
                                exportBillingEnergy(rowData.pkPiclbkqk);
                            }}
                            sx={{
                                textTransform: 'none',
                                fontSize: '0.875rem',
                ...
                // 如果 colRvhidn 既不是 0 也不是 1，则不渲染任何内容
                return null;
            }
        },
    ];
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
        jchzjhug: props.queryParams?.jchzjhug ?? '',
        dnsmnqeh: props.queryParams?.dnsmnqeh ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_jokhkp', params, {
            headers: {
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkPiclbkqk', val);
    ...
    return (
        <>
            ...
            <AddUpBillDialog
                title="Billed electricity table"
                open={isAddupBillDialogOpen}
                onClose={() => {
                    setIsAddupBillDialogOpen(false);
                }}
                children={WrapSoloFormNode(Parameterization('ViewTbPiclbkqkXvhrik', {
                    key: 'ViewTbPiclbkqkXvhrik',
                    readOnly: true,
                    initialData: { 'pkPiclbkqk': pkId },
