...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const [asfwmfcvOpen, setAsfwmfcvOpen] = useState(false);

    const columnData: ColumnDataProp[] = [
    			{
		            id: 'name', label: 'Group', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'deptcode', label: 'Number', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'citynode', label: '城市代码', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'cityname', label: '城市', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'adddtl', label: '详细地址', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'email', label: '邮箱', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'tel', label: '联系电话', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'autosp', label: 'Operation', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['id' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['id' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		gname: props.queryParams?.gname ?? '',
    		gcode: props.queryParams?.gcode ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/prefab_department_list', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: 'Group Information View',
                subheader: 'Group Information View',
                elem: Parameterization('PrefabDepartmentEdit', {
                    key: 'PrefabDepartmentEdit',
                    initialData: {
                       'gid': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: 'Group Management List',
                            subheader: 'Group Management List',
                            elem: Parameterization('PrefabDepartmentList', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/PrefabDepartmentEdit', {
                state: {
                    key: 'PrefabDepartmentEdit',
                    initialData: {
                       'gid': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/PrefabDepartmentList', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('gid', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/prefab_department_add', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
    ...
    const sfwmfcvHandler = () => {
        	setAsfwmfcvOpen(true);
    }
    ...
    return (
        <>
                        ...
                        <Box>
                        	<Tooltip title="Group Details" arrow>
	                            <Button
	                                ...
	                                onClick={sfwmfcvHandler}
	                                aria-describedby={'sfwmfcv'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
            ...
            <AddDataDialog
                ...
                open={asfwmfcvOpen}
                onClose={() => {
                    setAsfwmfcvOpen(false);
                }}
                children={WrapSoloFormNode(Parameterization('PrefabDepartmentAdd', {
                    key: 'PrefabDepartmentAdd',
                    initialData: {
