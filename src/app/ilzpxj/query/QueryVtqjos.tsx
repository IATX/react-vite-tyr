...
const DataTable: React.ForwardRefRenderFunction<PrefabRoleListRef, {page?: number, pageSize?: number, queryParams?: any}> = (props, ref) => {
    ...
    const columnData: ColumnDataProp[] = [
    			{
		            id: 'lvoezuwe', label: '开始日期', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'yhmyxvcn', label: '结束日期', minWidth: 150, align: 'center', type: 'string', truncate: true
		            				},	
    			{
		            id: 'ltqsvqts', label: '上网电量', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'zmdrspvi', label: '发电量', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'almepfwd', label: '售电量', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'uxmjmxgu', label: '结算金额', minWidth: 150, align: 'center', type: 'number', truncate: true
		            				},	
    			{
		            id: 'autosp', label: '操作', minWidth: 150, align: 'center', type: 'autosp', truncate: false, render: (val: any, rowData: Data, allData: Data[]) => {
		                return (<>
		                	<Button size='small' onClick={() => {
			                    handleEdit(rowData['pkJdelyxii' as keyof Data]);
				        	}}
				        	sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
		                    ...
		                    <Button size='small' color="error" onClick={() => {
		                        handleDelete(rowData['pkJdelyxii' as keyof Data]);
		                    }}
		                    sx={{
	                            textTransform: 'none',
	                            fontSize: '0.875rem',
    ...
    const [queryData, setQueryData] = useState<Record<string, string>>({
    		lvoezuwe: props.queryParams?.lvoezuwe ?? '',
    		yhmyxvcn: props.queryParams?.yhmyxvcn ?? '',
    });
    ...
    const fetchData = async () => {
        ...
        axios.post(VITE_JET_ASP_BPC_API + '/tablequery/listreacttable/query_vtqjos', params, {
            headers: {
    ...
    const handleEdit = (val: any) => {
		if (isInMainArea) {
            setCurrentBayContent({
                title: '电量合计表单',
                subheader: '电量合计表单',
                elem: Parameterization('ViewTbJdelyxiiRwhmui', {
                    key: 'ViewTbJdelyxiiRwhmui',
                    initialData: {
                       'pkJdelyxii': val
                    },
                    onCancel: (formData: any) => {
                        setCurrentBayContent({
                            title: '电费电量合计列表',
                            subheader: '电费电量合计列表',
                            elem: Parameterization('QueryVtqjos', {
								 page: page,
                                 pageSize: pageSize,
            ...
            navigate('app/ilzpxj/view/ViewTbJdelyxiiRwhmui', {
                state: {
                    key: 'ViewTbJdelyxiiRwhmui',
                    initialData: {
                       'pkJdelyxii': val
                    ...
                    onCancel: (formData: any) => {
                        navigate('app/ilzpxj/query/QueryVtqjos', {
                			state: {
                				page: page,
                                pageSize: pageSize,
                                queryParams: queryData
    ...
    const handleDelete = async (val: any) => {
        ...
        if (confirmed) {
            ...
            formData.append('pkJdelyxii', val);

            axios.post(VITE_JET_ASP_BPC_API + '/tableview/deleteformdata/view_tb_jdelyxii_rwhmui', formData, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'grooveToken': token
                }
            ...
            <Box>
                ...
                <Box className="mb-2" sx={{
                    ...
                    <Stack direction="row" spacing={2} alignItems="center">
                        <Box>
                        	<Tooltip title="电量合计表单" arrow>
	                            <Button
	                                variant="outlined"
	                                startIcon={<AddIcon sx={{ color: 'hsl(210, 100%, 45%);' }} />}
	                                onClick={unqypejHandler}
	                                aria-describedby={'unqypej'}
	                                size="small"
	                                sx={{
	                                    color: 'hsl(215, 15%, 22%)',
	                                    borderRadius: '12px',
