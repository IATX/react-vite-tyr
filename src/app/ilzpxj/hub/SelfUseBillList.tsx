...
import { VisibilityOutlined, StorefrontTwoTone } from '@mui/icons-material';
...
// 商户电费账单行（字段沿用 Billcenter 的 MerchantData / query_kaeany）
interface MerchantData {
    pkJkywwxtl: number;
    pulfqxdt: number;   // 账单日期
    btvttbdp: string;   // 发电户号
    snebfkdc: string;   // 户名
    xwizrffi: string;   // 供电服务单位
    jgeyksaw: number;   // 本期电量
    fmjxqxwc: number;   // 本期电费
}
                    ...
                    <div className="flex justify-between items-start mb-8">
                        <div>
                            <div className="flex items-center gap-2">
                                <StorefrontTwoTone sx={{ fontSize: 20, color: '#2563eb' }} />
                                <h2 className="text-base font-bold text-gray-900 tracking-tight">自发自用账单</h2>
                            </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-gray-200">
                                    <th className={thStyle}>账单日期</th>
                                    <th className={thStyle}>发电户号</th>
                                    <th className={thStyle}>户名</th>
                                    <th className={thStyle}>供电服务单位</th>
                                    <th className={thStyle}>本期电量</th>
                                    <th className={thStyle}>本期电费</th>
                                ...
                                {tableData.length === 0 ? (
                                    ...
                                    tableData.map((data, index) => (
                                        <tr key={data.pkJkywwxtl ?? index} className="hover:bg-gray-50/50 transition-colors group">
                                            ...
                                            <td className={tdStyle}>{data.btvttbdp}</td>
                                            <td className={tdStyle}>{data.snebfkdc}</td>
                                            <td className={tdStyle}>{data.xwizrffi}</td>
                                            <td className={tdStyle}>{data.jgeyksaw}</td>
                                            <td className={tdStyle}>{data.fmjxqxwc}</td>
...
export default SelfUseBillList;
