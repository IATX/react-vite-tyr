...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkTcbogtyo !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_tcbogtyo_afmnqw/' + formData.pkTcbogtyo, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_tcbogtyo_afmnqw', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...prevData,
					pkTcbogtyo: response.data.data.pkTcbogtyo,
				}));
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
					    	...
					    	if (validationRules["ugngprmw"]) {
						    	const errorMsg = validationRules["ugngprmw"](formData.ugngprmw);
						
								if (errorMsg != '') {
						    		newErrors["ugngprmw"] = errorMsg;
						    	}
						    }
					    	if (validationRules["juxkhfnm"]) {
						    	const errorMsg = validationRules["juxkhfnm"](formData.juxkhfnm);
						
								if (errorMsg != '') {
						    		newErrors["juxkhfnm"] = errorMsg;
						    	}
						    }
					    	if (validationRules["yeokgzoj"]) {
						    	const errorMsg = validationRules["yeokgzoj"](formData.yeokgzoj);
						
								if (errorMsg != '') {
						    		newErrors["yeokgzoj"] = errorMsg;
						    	}
						    }
					    	if (validationRules["iqxzxqxx"]) {
						    	const errorMsg = validationRules["iqxzxqxx"](formData.iqxzxqxx);
						
								if (errorMsg != '') {
						    		newErrors["iqxzxqxx"] = errorMsg;
						    	}
						    }
					    	if (validationRules["naqjbeoc"]) {
						    	const errorMsg = validationRules["naqjbeoc"](formData.naqjbeoc);
						
								if (errorMsg != '') {
						    		newErrors["naqjbeoc"] = errorMsg;
						    	}
						    }
					    	if (validationRules["dvuiumlm"]) {
						    	const errorMsg = validationRules["dvuiumlm"](formData.dvuiumlm);
						
								if (errorMsg != '') {
						    		newErrors["dvuiumlm"] = errorMsg;
						    	}
						    }
					    	if (validationRules["cwthkoid"]) {
						    	const errorMsg = validationRules["cwthkoid"](formData.cwthkoid);
						
								if (errorMsg != '') {
						    		newErrors["cwthkoid"] = errorMsg;
						    	}
						    }
					    	if (validationRules["thdvlhoj"]) {
						    	const errorMsg = validationRules["thdvlhoj"](formData.thdvlhoj);
						
								if (errorMsg != '') {
						    		newErrors["thdvlhoj"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_tcbogtyo_afmnqw" onSubmit={handleSubmit}>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_ugngprmw" className={labelStyle}>
	                发电单位
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['ugngprmw'] = el;
												}}>
		              <TextField
		                    id="tb_tcbogtyo_ugngprmw"
		                    name="ugngprmw"
		                    value={formData.ugngprmw || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.ugngprmw ? true : false}
		                    helperText={errors.ugngprmw}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_juxkhfnm" className={labelStyle}>
	                发电户号
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['juxkhfnm'] = el;
												}}>
		              <TextField
		                    id="tb_tcbogtyo_juxkhfnm"
		                    name="juxkhfnm"
		                    value={formData.juxkhfnm || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.juxkhfnm ? true : false}
		                    helperText={errors.juxkhfnm}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_yeokgzoj" className={labelStyle}>
	                发电地址
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['yeokgzoj'] = el;
												}}>
		              <TextField
		                    id="tb_tcbogtyo_yeokgzoj"
		                    name="yeokgzoj"
		                    value={formData.yeokgzoj || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.yeokgzoj ? true : false}
		                    helperText={errors.yeokgzoj}
		                    slotProps={{
					            input: {
		                  ...
		                  />
      					    	...
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>用电统计</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle}>
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_iqxzxqxx" className={labelStyle}>
	                账单日期
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['iqxzxqxx'] = el;
												}}>
		               <DatePicker
                          name="iqxzxqxx"
                          value={formData.iqxzxqxx ? dayjs(formData.iqxzxqxx) : null}
                          views={['year', 'month', 'day']}
                          onChange={(newValue: Dayjs | null)=> handleDateChange('iqxzxqxx', newValue)}
                          sx={{
                            '.Mui-focused': {
                          ...
                          slotProps={{
                            ...
                            textField: {
                              id: 'tb_tcbogtyo_iqxzxqxx',
                              error: !!errors['iqxzxqxx'],
                              helperText: errors['iqxzxqxx'],
                            },
                          }}
                       />
	              </FormControl>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_naqjbeoc" className={labelStyle}>
	                总发电量（kWh）
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['naqjbeoc'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_tcbogtyo_naqjbeoc"
		                    name="naqjbeoc"
		                    value={formData.naqjbeoc || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.naqjbeoc ? true : false}
		                    helperText={errors.naqjbeoc}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_dvuiumlm" className={labelStyle}>
	                上网电量（kWh）
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['dvuiumlm'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_tcbogtyo_dvuiumlm"
		                    name="dvuiumlm"
		                    value={formData.dvuiumlm || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.dvuiumlm ? true : false}
		                    helperText={errors.dvuiumlm}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_cwthkoid" className={labelStyle}>
	                关联用电户号
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['cwthkoid'] = el;
												}}>
		              <TextField
		                    id="tb_tcbogtyo_cwthkoid"
		                    name="cwthkoid"
		                    value={formData.cwthkoid || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.cwthkoid ? true : false}
		                    helperText={errors.cwthkoid}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_tcbogtyo_thdvlhoj" className={labelStyle}>
	                关联用电户用电量
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['thdvlhoj'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_tcbogtyo_thdvlhoj"
		                    name="thdvlhoj"
		                    value={formData.thdvlhoj || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.thdvlhoj ? true : false}
		                    helperText={errors.thdvlhoj}
		                    slotProps={{
					            input: {
