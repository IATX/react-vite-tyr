...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.rid !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/prefab_role_edit/' + formData.rid, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/prefab_role_edit', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_prefab_role_edit" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>Basic Information</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle}>
			<div className={oneColumnStyle}>
	            <label htmlFor="ISYS_ROLE_rname" className={labelStyle}>
	                Name
	            </label>
	            <div className="mt-2">
			...
			<input type="hidden" name="rkey" placeholder="" value={formData.rkey || ''} />
	    	<div className={oneRowStyle}>
	            <label htmlFor="ISYS_ROLE_rmemo" className={labelStyle}>
	                Description
	            </label>
	            <div className="mt-2">
	            	<FormControl fullWidth ref={(el) => {
						if (el) fieldRefs.current['rmemo'] = el;
					}}>
		               	<TextField
		                    multiline
		                    rows={4}
		                    id="ISYS_ROLE_rmemo"
		                    name="rmemo"
		                    value={formData.rmemo || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    fullWidth
		                    variant="outlined"
		                    error={errors.rmemo ? true : false}
		                    helperText={errors.rmemo}
		                    slotProps={{
					            input: {
			...
			<input type="hidden" name="rrv" placeholder="" value={formData.rrv || ''} />
			<input type="hidden" name="raccede" placeholder="" value={formData.raccede || ''} />
								    </div>
						       </div>
					</div>
