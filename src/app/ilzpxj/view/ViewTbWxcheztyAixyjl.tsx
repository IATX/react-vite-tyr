...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkWxchezty !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_wxchezty_aixyjl/' + formData.pkWxchezty, {}, {
				headers: {
		      ...
		      async function loadGlobalId() {
		        ...
		        try {
		          const result = await FetchPreloadPkId(token, 'view_tb_wxchezty_aixyjl', formData['TB_WXCHEZTY_org.limp.basework.impl.PreloadFakeId']);
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_wxchezty_aixyjl', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_wxchezty_aixyjl" onSubmit={handleSubmit}>
