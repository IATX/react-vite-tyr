...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.mid !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/prefab_module_add/' + formData.mid, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/prefab_module_add', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_prefab_module_add" onSubmit={handleSubmit}>
			...
			<input type="hidden" name="msort" placeholder="" value={formData.msort || ''} />
								    </div>
						       </div>
					</div>
