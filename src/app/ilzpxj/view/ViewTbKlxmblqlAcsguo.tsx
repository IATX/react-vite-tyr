...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkKlxmblql !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_klxmblql_acsguo/' + formData.pkKlxmblql, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_klxmblql_acsguo', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
					    	...
					    	if (validationRules["zggaznvo"]) {
						    	const errorMsg = validationRules["zggaznvo"](formData.zggaznvo);
						
								if (errorMsg != '') {
						    		newErrors["zggaznvo"] = errorMsg;
						    	}
						    }
					    	if (validationRules["tycssopm"]) {
						    	const errorMsg = validationRules["tycssopm"](formData.tycssopm);
						
								if (errorMsg != '') {
						    		newErrors["tycssopm"] = errorMsg;
						    	}
						    }
					    	if (validationRules["cisroanl"]) {
						    	const errorMsg = validationRules["cisroanl"](formData.cisroanl);
						
								if (errorMsg != '') {
						    		newErrors["cisroanl"] = errorMsg;
						    	}
						    }
					    	if (validationRules["rszqmgqi"]) {
						    	const errorMsg = validationRules["rszqmgqi"](formData.rszqmgqi);
						
								if (errorMsg != '') {
						    		newErrors["rszqmgqi"] = errorMsg;
						    	}
						    }
					    	if (validationRules["uwuwfqsu"]) {
						    	const errorMsg = validationRules["uwuwfqsu"](formData.uwuwfqsu);
						
								if (errorMsg != '') {
						    		newErrors["uwuwfqsu"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_klxmblql_acsguo" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>计费电量详情</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle}>
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_uwjkokxo" className={labelStyle}>
	                开始日期
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['uwjkokxo'] = el;
												}}>
		               <DatePicker
                          name="uwjkokxo"
                          value={formData.uwjkokxo ? dayjs(formData.uwjkokxo) : null}
                          views={['year', 'month', 'day']}
                          onChange={(newValue: Dayjs | null)=> handleDateChange('uwjkokxo', newValue)}
                          sx={{
                            '.Mui-focused': {
                          ...
                          slotProps={{
                            ...
                            textField: {
                              id: 'tb_klxmblql_uwjkokxo',
                              error: !!errors['uwjkokxo'],
                              helperText: errors['uwjkokxo'],
                            },
                          }}
                       />
	              </FormControl>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_gpsjckty" className={labelStyle}>
	                截至日期
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['gpsjckty'] = el;
												}}>
		               <DatePicker
                          name="gpsjckty"
                          value={formData.gpsjckty ? dayjs(formData.gpsjckty) : null}
                          views={['year', 'month', 'day']}
                          onChange={(newValue: Dayjs | null)=> handleDateChange('gpsjckty', newValue)}
                          sx={{
                            '.Mui-focused': {
                               borderColor: 'oklch(54.6% 0.245 262.881) !important',
                            },
                          ...
                          slotProps={{
                            ...
                            textField: {
                              id: 'tb_klxmblql_gpsjckty',
                              error: !!errors['gpsjckty'],
                              helperText: errors['gpsjckty'],
                            },
                          }}
                       />
	              </FormControl>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_zggaznvo" className={labelStyle}>
	                尖峰
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['zggaznvo'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_klxmblql_zggaznvo"
		                    name="zggaznvo"
		                    value={formData.zggaznvo || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.zggaznvo ? true : false}
		                    helperText={errors.zggaznvo}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_tycssopm" className={labelStyle}>
	                峰
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['tycssopm'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_klxmblql_tycssopm"
		                    name="tycssopm"
		                    value={formData.tycssopm || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.tycssopm ? true : false}
		                    helperText={errors.tycssopm}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_cisroanl" className={labelStyle}>
	                平
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['cisroanl'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_klxmblql_cisroanl"
		                    name="cisroanl"
		                    value={formData.cisroanl || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.cisroanl ? true : false}
		                    helperText={errors.cisroanl}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_rszqmgqi" className={labelStyle}>
	                谷
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['rszqmgqi'] = el;
												}}>
		              <TextField
		                    type="number"
		                    id="tb_klxmblql_rszqmgqi"
		                    name="rszqmgqi"
		                    value={formData.rszqmgqi || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.rszqmgqi ? true : false}
		                    helperText={errors.rszqmgqi}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_uwuwfqsu" className={labelStyle}>
	                计量点分类
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['uwuwfqsu'] = el;
												}}>
		              <TextField
		                    id="tb_klxmblql_uwuwfqsu"
		                    name="uwuwfqsu"
		                    value={formData.uwuwfqsu || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.uwuwfqsu ? true : false}
		                    helperText={errors.uwuwfqsu}
		                    slotProps={{
					            input: {
	    	...
	    	<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_kgnfrrnm" className={labelStyle}>
	                有效性
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth size="small"
	               error={!!errors['kgnfrrnm']}
	               ref={(el) => {
							if (el) fieldRefs.current['kgnfrrnm'] = el;
						}}
	               >
                      <Select
                        id="tb_klxmblql_kgnfrrnm"
                        name='kgnfrrnm'
                        value={formData.kgnfrrnm || ''}
                        onChange={(e) => handleSelectChange(e, [{'id':'Affirmative','name':'有效'},{'id':'Negative','name':'无效'},{'id':'Pending','name':'待确认'},] , 'pgewnhus')}
                      ...
                      <FormHelperText>{errors['kgnfrrnm']}</FormHelperText>
                    </FormControl>
			     </div>
	        </div>
			<input type="hidden" name="pgewnhus" placeholder="" value={formData.pgewnhus || ''} />
	    	<div className={oneColumnStyle}>
	            <label htmlFor="tb_klxmblql_oekmlfur" className={labelStyle}>
	                备注
	            </label>
	            <div className="mt-2">
	            	<FormControl fullWidth ref={(el) => {
						if (el) fieldRefs.current['oekmlfur'] = el;
					}}>
		               	<TextField
		                    multiline
		                    rows={4}
		                    id="tb_klxmblql_oekmlfur"
		                    name="oekmlfur"
		                    value={formData.oekmlfur || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    fullWidth
		                    variant="outlined"
		                    error={errors.oekmlfur ? true : false}
		                    helperText={errors.oekmlfur}
		                    slotProps={{
					            input: {
