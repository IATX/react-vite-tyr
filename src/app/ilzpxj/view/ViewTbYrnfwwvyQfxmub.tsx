...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	const cancelStyle = "rounded-md bg-gray-100 px-3 py-2 text-sm font-semibold shadow-xs hover:bg-gray-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600";
  	    		...
  	    		const yhpluwtg_fileUploaderRef = useRef<FileUploaderHandles | null>(null);
    ...
    useEffect(() => {
		...
		if (formData.pkYrnfwwvy !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_yrnfwwvy_qfxmub/' + formData.pkYrnfwwvy, {}, {
				headers: {
		      ...
		      async function loadGlobalId() {
		        ...
		        try {
		          const result = await FetchPreloadPkId(token, 'view_tb_yrnfwwvy_qfxmub', formData['TB_YRNFWWVY_org.limp.basework.impl.PreloadFakeId']);
		          
			    		setFormData((prevData: any) => ({ ...prevData, ['TB_YRNFWWVY_org.limp.basework.impl.PreloadFakeId']: result['TB_YRNFWWVY_org.limp.basework.impl.PreloadFakeId'] }));
		        } catch (error) {
		          if (error instanceof Error) {
						const wrapError = error as { response?: { status: number, data: any } };
						if (wrapError.response?.status == 400) {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const validationRules = {
			    "yhpluwtg": () => {
			      const isFileUploaderValid = yhpluwtg_fileUploaderRef.current?.validate();
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_yrnfwwvy_qfxmub', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...prevData,
					pkYrnfwwvy: response.data.data.pkYrnfwwvy,
				}));
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
				    		...
				    		if (validationRules["yhpluwtg"]) {
						    	const errorMsg = validationRules["yhpluwtg"]();
						
								if (errorMsg != '') {
						    		newErrors["yhpluwtg"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_yrnfwwvy_qfxmub" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>商户电费账单</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle + ` sm:grid-cols-2`}>
			<div className={oneColumnStyle + ` hidden`}>
	            <label htmlFor="tb_yrnfwwvy_qeyiqstx" className={labelStyle}>
	                标题
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.qeyiqstx || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['qeyiqstx'] = el;
												}}>
		              <TextField
		                    id="tb_yrnfwwvy_qeyiqstx"
		                    name="qeyiqstx"
		                    value={formData.qeyiqstx || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.qeyiqstx ? true : false}
		                    helperText={errors.qeyiqstx}
		                    slotProps={{
					            input: {
			...
			<div className={oneRowStyle + ` sm:col-span-2`} >
			    <label htmlFor="tb_yrnfwwvy_yhpluwtg" className={labelStyle}>
			        账单文件
			    </label>
			    <div className={uploadStyle}>
	                  <FileUploader
		                    ref={yhpluwtg_fileUploaderRef}
		                    id="tb_yrnfwwvy_yhpluwtg"
		                    name="yhpluwtg"
		                    label="DOC, IMAGE, ZIP up to 10MB, up to 5 files"
		                    rtn="TB_YRNFWWVY"
		                    rfn="yhpluwtg"
		                    rfpk={formData['TB_YRNFWWVY_org.limp.basework.impl.PreloadFakeId']}
		                    cfn={''}
		                    annexFiles={formData['yhpluwtg']}
	                  />
                </div>
			</div>
