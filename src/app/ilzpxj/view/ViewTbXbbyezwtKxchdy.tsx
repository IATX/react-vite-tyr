...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkXbbyezwt !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_xbbyezwt_kxchdy/' + formData.pkXbbyezwt, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_xbbyezwt_kxchdy', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
					    	...
					    	if (validationRules["mrvqpphi"]) {
						    	const errorMsg = validationRules["mrvqpphi"](formData.mrvqpphi);
						
								if (errorMsg != '') {
						    		newErrors["mrvqpphi"] = errorMsg;
						    	}
						    }
					    	if (validationRules["bwblkhay"]) {
						    	const errorMsg = validationRules["bwblkhay"](formData.bwblkhay);
					    	...
					    	if (validationRules["deimigjs"]) {
						    	const errorMsg = validationRules["deimigjs"](formData.deimigjs);
						
								if (errorMsg != '') {
						    		newErrors["deimigjs"] = errorMsg;
						    	}
						    }
					    	if (validationRules["jasosmzz"]) {
						    	const errorMsg = validationRules["jasosmzz"](formData.jasosmzz);
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_xbbyezwt_kxchdy" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>客户信息</h2>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_xbbyezwt_deimigjs" className={labelStyle}>
	                服务单位
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.deimigjs || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['deimigjs'] = el;
												}}>
		              <TextField
		                    id="tb_xbbyezwt_deimigjs"
		                    name="deimigjs"
		                    value={formData.deimigjs || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.deimigjs ? true : false}
		                    helperText={errors.deimigjs}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_xbbyezwt_armiyjlh" className={labelStyle}>
	                联系人
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.armiyjlh || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['armiyjlh'] = el;
												}}>
		              <TextField
		                    id="tb_xbbyezwt_armiyjlh"
		                    name="armiyjlh"
		                    value={formData.armiyjlh || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.armiyjlh ? true : false}
		                    helperText={errors.armiyjlh}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_xbbyezwt_eznomrac" className={labelStyle}>
	                联系电话
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.eznomrac || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['eznomrac'] = el;
												}}>
		              <TextField
		                    id="tb_xbbyezwt_eznomrac"
		                    name="eznomrac"
		                    value={formData.eznomrac || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.eznomrac ? true : false}
		                    helperText={errors.eznomrac}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_xbbyezwt_cejacmfl" className={labelStyle}>
	                邮编
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.cejacmfl || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['cejacmfl'] = el;
												}}>
		              <TextField
		                    id="tb_xbbyezwt_cejacmfl"
		                    name="cejacmfl"
		                    value={formData.cejacmfl || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.cejacmfl ? true : false}
		                    helperText={errors.cejacmfl}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_xbbyezwt_gagumdjb" className={labelStyle}>
	                联系地址
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.gagumdjb || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['gagumdjb'] = el;
												}}>
		              <TextField
		                    id="tb_xbbyezwt_gagumdjb"
		                    name="gagumdjb"
		                    value={formData.gagumdjb || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.gagumdjb ? true : false}
		                    helperText={errors.gagumdjb}
		                    slotProps={{
					            input: {
