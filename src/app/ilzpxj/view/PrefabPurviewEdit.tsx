...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pid !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/prefab_purview_edit/' + formData.pid, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/prefab_purview_edit', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
					    	...
					    	if (validationRules["rname"]) {
						    	const errorMsg = validationRules["rname"](formData.rname);
						
								if (errorMsg != '') {
						    		newErrors["rname"] = errorMsg;
						    	}
						    }
					    	if (validationRules["oname"]) {
						    	const errorMsg = validationRules["oname"](formData.oname);
						
								if (errorMsg != '') {
						    		newErrors["oname"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_prefab_purview_edit" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>Permission Form</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle}>
			<div className={oneColumnStyle}>
	            <label htmlFor="ISYS_PURVIEW_rname" className={labelStyle}>
	                Role Name
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['rname'] = el;
												}}>
		              <TextField
		                    id="ISYS_PURVIEW_rname"
		                    name="rname"
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="ISYS_PURVIEW_oname" className={labelStyle}>
	                Object Name
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['oname'] = el;
												}}>
		              <TextField
		                    id="ISYS_PURVIEW_oname"
		                    name="oname"
		                    value={formData.oname || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.oname ? true : false}
		                    helperText={errors.oname}
		                    slotProps={{
					            input: {
	    	...
	    	<div className={oneColumnStyle}>
	            <label htmlFor="ISYS_PURVIEW_purview" className={labelStyle}>
	                权限
	            </label>
	            <div className="mt-2">
	              <FormControl fullWidth size="small"
	               error={!!errors['purview']}
	               ref={(el) => {
						if (el) fieldRefs.current['purview'] = el;
					}}
	               >
                      <RadioGroup
					        row
					        aria-labelledby="purview-row-radio-buttons-group-label"
					        name="purview"
					        value={formData.purview}
							onChange={(e) => { handleRadioChange(e, [{'id':'-1','name':'不能访问'},{'id':'0','name':'可以访问'},] ) }}
					      >
				    	   	   <FormControlLabel value="-1" control={<Radio />} label="不能访问"   />
				    	   	   <FormControlLabel value="0" control={<Radio />} label="可以访问"   />
					  </RadioGroup>
                      <FormHelperText>{errors['purview']}</FormHelperText>
                    </FormControl>
			     </div>
	        </div> 
								    </div>
