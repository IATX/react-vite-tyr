...
import { FormControl, FormControlLabel, FormHelperText, MenuItem, Radio, Checkbox, RadioGroup, Select, TextField, type SelectChangeEvent, FormGroup, InputAdornment, Typography, Box, CircularProgress } from '@mui/material';
...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkPiclbkqk !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_piclbkqk_xvhrik/' + formData.pkPiclbkqk, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_piclbkqk_xvhrik', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_piclbkqk_xvhrik" onSubmit={handleSubmit}>
		                  ...
		                  />
      					    	...
      					    	<div className="listCardStyle">	
	      					    	<table className={tableStyle}>
	      					    		<caption className={tableCaptionStyle}>计费电量合计列表</caption>
	      					    		<thead>
	      					    			<tr>
	      					    		...
	      					    		<tbody className={tableTbodyStyle}>
											{formData.listYeytzf.map((item: any) => (
      					    	...
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>计费电量汇总详情</h2>
		                            <p className={groupDescriptionStyle}>
