...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	const cancelStyle = "rounded-md bg-gray-100 px-3 py-2 text-sm font-semibold shadow-xs hover:bg-gray-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600";
  	    		...
  	    		const avsbghww_fileUploaderRef = useRef<FileUploaderHandles | null>(null);
	...
	const [formData, setFormData] = useState(() => {
		const defaultData = {
			        		...
			        		colLvmmsu: '',
	    	pkDhnkqhqg: '',
		    		'TB_DHNKQHQG_org.limp.basework.impl.PreloadFakeId': '',
    ...
    useEffect(() => {
		...
		if (formData.pkDhnkqhqg !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_dhnkqhqg_qdoeqk/' + formData.pkDhnkqhqg, {}, {
				headers: {
		      ...
		      async function loadGlobalId() {
		        ...
		        try {
		          const result = await FetchPreloadPkId(token, 'view_tb_dhnkqhqg_qdoeqk', formData['TB_DHNKQHQG_org.limp.basework.impl.PreloadFakeId']);
		          
			    		setFormData((prevData: any) => ({ ...prevData, ['TB_DHNKQHQG_org.limp.basework.impl.PreloadFakeId']: result['TB_DHNKQHQG_org.limp.basework.impl.PreloadFakeId'] }));
		        } catch (error) {
		          if (error instanceof Error) {
						const wrapError = error as { response?: { status: number, data: any } };
						if (wrapError.response?.status == 400) {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const validationRules = {
			    ...
			    "avsbghww": () => {
			      const isFileUploaderValid = avsbghww_fileUploaderRef.current?.validate();
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_dhnkqhqg_qdoeqk', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...prevData,
					pkDhnkqhqg: response.data.data.pkDhnkqhqg,
				}));
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
					    	...
					    	if (validationRules["colVwspvk"]) {
						    	const errorMsg = validationRules["colVwspvk"](formData.colVwspvk);
						
								if (errorMsg != '') {
						    		newErrors["colVwspvk"] = errorMsg;
						    	}
						    }
					    	if (validationRules["colLvmmsu"]) {
						    	const errorMsg = validationRules["colLvmmsu"](formData.colLvmmsu);
						
								if (errorMsg != '') {
						    		newErrors["colLvmmsu"] = errorMsg;
						    	}
						    }
				    		if (validationRules["avsbghww"]) {
						    	const errorMsg = validationRules["avsbghww"]();
						
								if (errorMsg != '') {
						    		newErrors["avsbghww"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_dhnkqhqg_qdoeqk" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>代理购电电价表</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle + ` sm:grid-cols-2`}>
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_dhnkqhqg_colVwspvk" className={labelStyle}>
	                执行时间起
	            </label>
	            <div className="mt-2">
	            	{isViewReadOnly ? (
	            		<Typography variant="body2" gutterBottom>{new Date(formData.colVwspvk).toLocaleDateString() || ''}</Typography>
	            	) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['colVwspvk'] = el;
												}}>
		               <DatePicker
                          name="colVwspvk"
                          format="YYYY-MM-DD"
                          value={formData.colVwspvk ? dayjs(formData.colVwspvk) : null}
                          views={['year', 'month', 'day']}
                          onChange={(newValue: Dayjs | null)=> handleDateChange('colVwspvk', newValue)}
                          sx={{
                            '.Mui-focused': {
                          ...
                          slotProps={{
                            ...
                            textField: {
                              id: 'tb_dhnkqhqg_colVwspvk',
                              error: !!errors['colVwspvk'],
                              helperText: errors['colVwspvk'],
                            },
                          }}
                       />
	              </FormControl>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_dhnkqhqg_colLvmmsu" className={labelStyle}>
	                执行时间至
	            </label>
	            <div className="mt-2">
	            	{isViewReadOnly ? (
	            		<Typography variant="body2" gutterBottom>{new Date(formData.colLvmmsu).toLocaleDateString() || ''}</Typography>
	            	) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['colLvmmsu'] = el;
												}}>
		               <DatePicker
                          name="colLvmmsu"
                          format="YYYY-MM-DD"
                          value={formData.colLvmmsu ? dayjs(formData.colLvmmsu) : null}
                          views={['year', 'month', 'day']}
                          onChange={(newValue: Dayjs | null)=> handleDateChange('colLvmmsu', newValue)}
                          sx={{
                            '.Mui-focused': {
                               borderColor: 'oklch(54.6% 0.245 262.881) !important',
                            },
                          ...
                          slotProps={{
                            ...
                            textField: {
                              id: 'tb_dhnkqhqg_colLvmmsu',
                              error: !!errors['colLvmmsu'],
                              helperText: errors['colLvmmsu'],
                            },
                          }}
                       />
	              </FormControl>
			...
			<div className={oneRowStyle + ` sm:col-span-2`}>
			    <label htmlFor="tb_dhnkqhqg_avsbghww" className={labelStyle}>
			        电价表
			    </label>
			    <div className={uploadStyle}>
	                  <FileUploader
		                    ref={avsbghww_fileUploaderRef}
		                    id="tb_dhnkqhqg_avsbghww"
		                    name="avsbghww"
		                    label="DOC, IMAGE, ZIP up to 10MB, up to 5 files"
		                    rtn="TB_DHNKQHQG"
		                    rfn="avsbghww"
		                    rfpk={formData['TB_DHNKQHQG_org.limp.basework.impl.PreloadFakeId']}
		                    cfn={''}
		                    annexFiles={formData['avsbghww']}
	                  />
                </div>
			</div>
								    </div>
