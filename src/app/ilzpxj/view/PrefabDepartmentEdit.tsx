...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.gid !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/prefab_department_edit/' + formData.gid, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/prefab_department_edit', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_prefab_department_edit" onSubmit={handleSubmit}>
