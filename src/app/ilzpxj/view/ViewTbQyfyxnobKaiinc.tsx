...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	const [formData, setFormData] = useState(() => {
    	...
    	const [focusedlistSnnbalRowId, setFocusedlistSnnbalRowId] = useState<string | null>(null);
		const [editinglistSnnbalRowId, setEditinglistSnnbalRowId] = useState<string | null>(null);
		const [isConfirmDeletionlistSnnbalRowOpen, setIsConfirmDeletionlistSnnbalRowOpen] = useState(false);	
    ...
    useEffect(() => {
		...
		if (formData.pkQyfyxnob !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_qyfyxnob_kaiinc/' + formData.pkQyfyxnob, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_qyfyxnob_kaiinc', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...prevData,
					pkQyfyxnob: response.data.data.pkQyfyxnob,
				}));
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_qyfyxnob_kaiinc" onSubmit={handleSubmit}>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_qyfyxnob_fpbrgkgd" className={labelStyle}>
	                编制单位
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.fpbrgkgd || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['fpbrgkgd'] = el;
												}}>
		              <TextField
		                    id="tb_qyfyxnob_fpbrgkgd"
		                    name="fpbrgkgd"
		                    value={formData.fpbrgkgd || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.fpbrgkgd ? true : false}
		                    helperText={errors.fpbrgkgd}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_qyfyxnob_tgkyimda" className={labelStyle}>
	                结算年月
	            </label>
	            <div className="mt-2">
	            	{isViewReadOnly ? (
	            		<Typography variant="body2" gutterBottom>{new Date(formData.tgkyimda).toLocaleDateString() || ''}</Typography>
	            	) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['tgkyimda'] = el;
												}}>
		               <DatePicker
                          name="tgkyimda"
                          format="YYYY-MM-DD"
                          value={formData.tgkyimda ? dayjs(formData.tgkyimda) : null}
                          views={['year', 'month', 'day']}
                          onChange={(newValue: Dayjs | null)=> handleDateChange('tgkyimda', newValue)}
                          sx={{
                            '.Mui-focused': {
                          ...
                          slotProps={{
                            ...
                            textField: {
                              id: 'tb_qyfyxnob_tgkyimda',
                              error: !!errors['tgkyimda'],
                              helperText: errors['tgkyimda'],
                            },
                          }}
                       />
	              </FormControl>
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_qyfyxnob_rsyhgxls" className={labelStyle}>
	                核定人
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.rsyhgxls || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['rsyhgxls'] = el;
												}}>
		              <TextField
		                    id="tb_qyfyxnob_rsyhgxls"
		                    name="rsyhgxls"
		                    value={formData.rsyhgxls || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.rsyhgxls ? true : false}
		                    helperText={errors.rsyhgxls}
		                    slotProps={{
					            input: {
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_qyfyxnob_lqtbkgxz" className={labelStyle}>
	                审核人
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.lqtbkgxz || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['lqtbkgxz'] = el;
												}}>
		              <TextField
		                    id="tb_qyfyxnob_lqtbkgxz"
		                    name="lqtbkgxz"
		                    value={formData.lqtbkgxz || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.lqtbkgxz ? true : false}
		                    helperText={errors.lqtbkgxz}
		                    slotProps={{
					            input: {
	    	...
	    	<div className={oneColumnStyle}>
	            <label htmlFor="tb_qyfyxnob_actvvgwn" className={labelStyle}>
	                说明
	            </label>
	            <div className="mt-2">
	            	{isViewReadOnly ? (
	            		<Typography variant="body2" gutterBottom>{formData.actvvgwn || ''}</Typography>
	            	) : (
	            	<FormControl fullWidth ref={(el) => {
						if (el) fieldRefs.current['actvvgwn'] = el;
					}}>
		               	<TextField
		                    multiline
		                    rows={4}
		                    id="tb_qyfyxnob_actvvgwn"
		                    name="actvvgwn"
		                    value={formData.actvvgwn || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    fullWidth
		                    variant="outlined"
		                    error={errors.actvvgwn ? true : false}
		                    helperText={errors.actvvgwn}
		                    slotProps={{
					            input: {
	    	...
	    	<div className={oneColumnStyle}>
	            <label htmlFor="tb_qyfyxnob_vfjqzhvx" className={labelStyle}>
	                状态
	            </label>
	            <div className="mt-2">
	            	{isViewReadOnly ? (
	            		<Typography variant="body2" gutterBottom>{formData.dgbqhwta || ''}</Typography>
	            	) : (
	              <FormControl fullWidth size="small"
	               error={!!errors['vfjqzhvx']}
	               ref={(el) => {
							if (el) fieldRefs.current['vfjqzhvx'] = el;
						}}
	               >
                      <Select
                        id="tb_qyfyxnob_vfjqzhvx"
                        name='vfjqzhvx'
                        value={formData.vfjqzhvx || ''}
                        onChange={(e) => handleSelectChange(e, [{'id':'0','name':'待审核'},{'id':'-1','name':'未通过'},{'id':'1','name':'通过'},] , 'dgbqhwta')}
                      ...
                      >
                        <MenuItem value="">
                          <em>&nbsp;</em>
				    		...
				    		<MenuItem value='-1'>未通过</MenuItem>
				    		<MenuItem value='1'>通过</MenuItem>
                      </Select>
                      <FormHelperText>{errors['vfjqzhvx']}</FormHelperText>
                    </FormControl>
                    )}
			     </div>
	        </div>
			<input type="hidden" name="dgbqhwta" placeholder="" value={formData.dgbqhwta || ''} />
								    </div>
						        </div>
      					    	<div className={listFitCardStyle}>	
	      					    	<table className={tableStyle}>
	      					    		...
	      					    		<div className="flex justify-between items-center w-full">
	      					    			<h2 className={groupTitleStyle}>结算单详情</h2>
		      					    		...
		      					    		{!!!isViewReadOnly && (
												<Button
													...
													startIcon={<AddIcon />}
													onClick={(e) => {
														setFocusedlistSnnbalRowId(null);
														setEditinglistSnnbalRowId(null);
	
														const newRow: any = {
	
														};
	
														setFormData((prevFormData: { listSnnbal: any; }) => ({
															...prevFormData,
															listSnnbal: [
																...prevFormData.listSnnbal,
																newRow,
															],
														}));
													}}
	      					    		...
	      					    		<thead>
	      					    			<tr>
	      					    				<th className={tableThStyle}>项目名称</th>
	      					    				<th className={tableThStyle}>当月数据-结算电量千瓦时</th>
	      					    				<th className={tableThStyle}>当月数据-代购电价元千瓦时</th>
	      					    				<th className={tableThStyle}>当月数据-结算电价不含税元千瓦时</th>
	      					    				<th className={tableThStyle}>当月数据-结算电价不含税元千瓦时</th>
	      					    				<th className={tableThStyle}>当月数据-结算电费</th>
	      					    				<th className={tableThStyle}>当月数据-营业收入</th>
	      					    				<th className={tableThStyle}>当月数据-销项税额</th>
	      					    				<th className={tableThStyle}>累计数据-结算电量千瓦时</th>
	      					    				<th className={tableThStyle}>累计数据-结算电价含税元千瓦时</th>
	      					    				<th className={tableThStyle}>累计数据-结算电价不含税元千瓦时</th>
	      					    				<th className={tableThStyle}>累计数据-结算电费</th>
	      					    				<th className={tableThStyle}>累计数据-营业收入</th>
	      					    				<th className={tableThStyle}>累计数据-销项税额</th>
	      					    				<th className={tableThStyle}>备注</th>
	      					    			 </tr>
	      					    		</thead>
	      					    		<tbody className={tableTbodyStyle}>
											{formData.listSnnbal.map((item: any, index: number) => {
												const rowIndex = 'row_' + index;
												
												const isFocused = rowIndex === focusedlistSnnbalRowId && !isViewReadOnly;
												const isEditing = rowIndex === editinglistSnnbalRowId && !isViewReadOnly;
												
												return (
													<tr 
													key={rowIndex}
													onClick={() => {
														if (!focusedlistSnnbalRowId || editinglistSnnbalRowId !== rowIndex) {
															setFocusedlistSnnbalRowId(rowIndex);
															setEditinglistSnnbalRowId(null);
														}
													}}
													className={[
														...
														isFocused ? 'bg-blue-50' : 'hover:bg-gray-50',
			      					    				...
			      					    				{isEditing ? (
															<RowInputRenderer
																...
																value={item.xqkxcacq}
																onChange={(newValue) => {
																	const updatedList = formData.listSnnbal.map((row: any, idx: number) => {
																	...
																	<ButtonGroup size="small" color="primary" orientation="vertical" sx={{ 
																		...
																		<Button startIcon={<InsertBeforeIcon />}
																			...
																			onClick={(e) => {
																				...
																				const currentList = formData.listSnnbal || [];
		
																				const newRow: any = {
																				};
		
																				let insertIndex = currentList.findIndex((row: any, idx: number) => ('row_' + idx) === focusedlistSnnbalRowId);
																		...
																		<Button startIcon={<InsertNextIcon />}
																			...
																			onClick={(e) => {
																				...
																				let focusedIndex = currentList.findIndex((row: any, idx: number) => ('row_' + idx) === focusedlistSnnbalRowId);
																		...
																		<Button startIcon={<EditIcon />}
																			...
																			onClick={(e) => {
																				...
																				setEditinglistSnnbalRowId(rowIndex);
																			}}
																			sx={{
																				justifyContent: 'flex-start'
																			}}
																		...
																		<Button startIcon={<DeleteIcon />}
																			...
																			onClick={(e) => {
																				...
																				setEditinglistSnnbalRowId(null);
																				setIsConfirmDeletionlistSnnbalRowOpen(true);
																			}}
																			sx={{
																				justifyContent: 'flex-start'
																			}}
	      					    	...
	      					    	<SimpleConfirmDialog
										open={isConfirmDeletionlistSnnbalRowOpen}
										onConfirm={() => {
											setIsConfirmDeletionlistSnnbalRowOpen(false);
		
											// 1. Find the focused element outside of setFormData.
											const foundElement = formData.listSnnbal.find((row: any, idx: number) => {
												const rindex = 'row_' + idx;
		
												return rindex == focusedlistSnnbalRowId;
											});
		
											// 2. Use setFormData uniformly for all immutable state updates.
											setFormData((prevData: any) => {
												// 2a. Filter the new data list array (remove the focused row).
												const newList = prevData.listSnnbal.filter((row: any, idx: number) => {
													const rindex = 'row_' + idx;
													return rindex !== focusedlistSnnbalRowId;
												});
												...
												let discardedDataList = [...(prevData.formDataBin?.listSnnbal || [])];
		
												// 2c. Handle primary key insertion logic.
												if (foundElement && foundElement['pkYsgdrhti'] !== null) {
												...
												return {
													...prevData,
													['listSnnbal']: newList, // Update data list
													formDataBin: {
														...prevData.formDataBin, // Preserve other possible key-value pairs in formDataBin
														listSnnbal: discardedDataList,       // Update the data List array in formDataBin
													},
												};
											});
