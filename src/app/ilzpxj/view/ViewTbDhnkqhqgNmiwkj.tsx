...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkDhnkqhqg !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_dhnkqhqg_nmiwkj/' + formData.pkDhnkqhqg, {}, {
				headers: {
		      ...
		      async function loadGlobalId() {
		        ...
		        try {
		          const result = await FetchPreloadPkId(token, 'view_tb_dhnkqhqg_nmiwkj', formData['TB_DHNKQHQG_org.limp.basework.impl.PreloadFakeId']);
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_dhnkqhqg_nmiwkj', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_dhnkqhqg_nmiwkj" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>代理购电价格表</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle + ` sm:grid-cols-2`}>
			<input type="hidden" name="pzggscif" placeholder="" value={formData.pzggscif || ''} />
