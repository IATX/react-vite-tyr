...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	const cancelStyle = "rounded-md bg-gray-100 px-3 py-2 text-sm font-semibold shadow-xs hover:bg-gray-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600";
  	    		...
  	    		const mgesntrc_fileUploaderRef = useRef<FileUploaderHandles | null>(null);
	...
	const [formData, setFormData] = useState(() => {
		const defaultData = {
			        		lzevroay: 'FixedPrice',
	    	pkWzbgmxdi: '',
		    		'TB_WZBGMXDI_org.limp.basework.impl.PreloadFakeId': '',
    ...
    useEffect(() => {
		...
		if (formData.pkWzbgmxdi !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_wzbgmxdi_koossp/' + formData.pkWzbgmxdi, {}, {
				headers: {
		      ...
		      async function loadGlobalId() {
		        ...
		        try {
		          const result = await FetchPreloadPkId(token, 'view_tb_wzbgmxdi_koossp', formData['TB_WZBGMXDI_org.limp.basework.impl.PreloadFakeId']);
		          
			    		setFormData((prevData: any) => ({ ...prevData, ['TB_WZBGMXDI_org.limp.basework.impl.PreloadFakeId']: result['TB_WZBGMXDI_org.limp.basework.impl.PreloadFakeId'] }));
		        } catch (error) {
		          if (error instanceof Error) {
						const wrapError = error as { response?: { status: number, data: any } };
						if (wrapError.response?.status == 400) {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const validationRules = {
			    "mgesntrc": () => {
			      const isFileUploaderValid = mgesntrc_fileUploaderRef.current?.validate();
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_wzbgmxdi_koossp', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...prevData,
					pkWzbgmxdi: response.data.data.pkWzbgmxdi,
				}));
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
				    		...
				    		if (validationRules["mgesntrc"]) {
						    	const errorMsg = validationRules["mgesntrc"]();
						
								if (errorMsg != '') {
						    		newErrors["mgesntrc"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_wzbgmxdi_koossp" onSubmit={handleSubmit}>
			...
			<input type="hidden" name="lzevroay" placeholder="" value={formData.lzevroay || ''} />
			<div className={oneRowStyle + ` sm:col-span-2`}>
			    <label htmlFor="tb_wzbgmxdi_mgesntrc" className={labelStyle}>
			        账单文件
			    </label>
			    <div className={uploadStyle}>
	                  <FileUploader
		                    ref={mgesntrc_fileUploaderRef}
		                    id="tb_wzbgmxdi_mgesntrc"
		                    name="mgesntrc"
		                    label="DOC, IMAGE, ZIP up to 10MB, up to 5 files"
		                    rtn="TB_WZBGMXDI"
		                    rfn="mgesntrc"
		                    rfpk={formData['TB_WZBGMXDI_org.limp.basework.impl.PreloadFakeId']}
		                    cfn={''}
		                    annexFiles={formData['mgesntrc']}
	                  />
                </div>
			</div>
								    </div>
