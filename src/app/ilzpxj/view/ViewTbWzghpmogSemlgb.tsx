...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.pkWzghpmog !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_wzghpmog_semlgb/' + formData.pkWzghpmog, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_wzghpmog_semlgb', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (Object.keys(newErrors).length === 0) {
			...
			const [tqepibxbvMultiSelectKeys, setTqepibxbvMultiSelectKeys] = useState<string[]>([]);
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_wzghpmog_semlgb" onSubmit={handleSubmit}>
	    	...
	    	<div className={oneRowStyle}>
	            <label htmlFor="tb_wzghpmog_qepibxbv" className={labelStyle}>
	                商户
	            </label>
	            <div className="mt-2">
	            	{isViewReadOnly ? (
	            		<Typography variant="body2" gutterBottom>{formData.qepibxbv || ''}</Typography>
	            	) : (
	               	<FormControl fullWidth ref={(el) => {
						if (el) fieldRefs.current['qepibxbv'] = el;
					}}>
						<InputWithList
							placeholder="Select..."
							dataKey="query_jxzqzn"
							isMultiSelect={false}
							selectedKeys={tqepibxbvMultiSelectKeys}
							onChange={(keys: string[], values: string[]) => {
								setTqepibxbvMultiSelectKeys(keys);
							...
							pointId="pkXbbyezwt"
							pointName="mrvqpphi"
							id="tb_wzghpmog_qepibxbv"
							name="qepibxbv"
							value={formData.xdsjflqz || ''}
							size="small"
							variant="outlined"
							error={errors.qepibxbv ? true : false}
							helperText={errors.qepibxbv}
							slotProps={{
								input: {
	              ...
	              <FormControl fullWidth size="small"
	               ...
	               >
                      <RadioGroup
					      ...
					      >
				    	   	   <FormControlLabel value="P0001" control={<Radio />} label="代理购电工商业用户电价表" slotProps={{
    typography: { sx: { fontSize: '0.875rem' } } // 直接控制内部字体
  }} />
				    	   	   <FormControlLabel value="P0002" control={<Radio />} label="电费单电价"  slotProps={{
    typography: { sx: { fontSize: '0.875rem' } } // 直接控制内部字体
  }} />
					  </RadioGroup>
