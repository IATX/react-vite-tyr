...
import { useInitialActions } from '../js/view_tb_gvfrokeq_sfidnf';
...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	useEffect(() => {
		...
		if (formData.pkGvfrokeq !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_gvfrokeq_sfidnf/' + formData.pkGvfrokeq, {}, {
				headers: {
			...
			if (formData['gfumgrlg']) {
				setTgfumgrlgMultiSelectKeys([formData['gfumgrlg'] + '']);
			}
		}
	...
	const submitFormData = () => {
		...
		axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_gvfrokeq_sfidnf', trimObjectValues(formData), {
			headers: {
				'grooveToken': token
			}
		}).then(response => {
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (validationRules["gfumgrlg"]) {
			const errorMsg = validationRules["gfumgrlg"](formData.gfumgrlg);

			if (errorMsg != '') {
				newErrors["gfumgrlg"] = errorMsg;
			}
		}
	...
	const [tgfumgrlgMultiSelectKeys, setTgfumgrlgMultiSelectKeys] = useState<string[]>([]);
	...
	return (
		<>
			...
			<ThemeProvider theme={theme}>
				<form id="form_view_tb_gvfrokeq_sfidnf" onSubmit={handleSubmit}>
						...
						<div className={groupCardStyle}>
							...
							<div className={groupContentStyle + ` sm:grid-cols-2`}>
								<div className={oneColumnStyle}>
									<label htmlFor="tb_gvfrokeq_gfumgrlg" className={labelStyle}>
										商户
									</label>
									<div className="mt-2">
										{isViewReadOnly ? (
											<Typography variant="body2" gutterBottom>{formData.gfumgrlg || ''}</Typography>
										) : (
											<FormControl fullWidth ref={(el) => {
												if (el) fieldRefs.current['gfumgrlg'] = el;
											}}>
												<InputWithList
													placeholder="Select..."
													dataKey="query_jxzqzn"
													isMultiSelect={false}
													selectedKeys={tgfumgrlgMultiSelectKeys}
													onChange={(keys: string[], values: string[]) => {
														setTgfumgrlgMultiSelectKeys(keys);
													...
													size="small"
													variant="outlined"
													error={errors.gfumgrlg ? true : false}
													helperText={errors.gfumgrlg}
													slotProps={{
														input: {
								...
								<input type="hidden" name="tparorbm" placeholder="" value={formData.tparorbm || ''} />
								<input type="hidden" name="ynwjibye" placeholder="" value={formData.ynwjibye || ''} />
								<div className={oneColumnStyle}>
