...
export default function ViewPage<T extends object = { [key: string]: any }>({ initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
    ...
    useEffect(() => {
		...
		if (formData.rid !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/prefab_role_add/' + formData.rid, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		setDisabledAction(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/prefab_role_add', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_prefab_role_add" onSubmit={handleSubmit}>
