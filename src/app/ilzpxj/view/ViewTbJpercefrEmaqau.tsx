...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	const cancelStyle = "rounded-md bg-gray-100 px-3 py-2 text-sm font-semibold shadow-xs hover:bg-gray-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600";
  	    		...
  	    		const gfaazpnv_fileUploaderRef = useRef<FileUploaderHandles | null>(null);
	
	// Managing form data and validation errors with useState
	const [formData, setFormData] = useState(() => {
		const defaultData = {
			        		xahuwknf: 'ConsumptionRatio',
	    	pkJpercefr: '',
		    		'TB_JPERCEFR_org.limp.basework.impl.PreloadFakeId': '',
    ...
    useEffect(() => {
		...
		if (formData.pkJpercefr !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_jpercefr_emaqau/' + formData.pkJpercefr, {}, {
				headers: {
		      ...
		      async function loadGlobalId() {
		        ...
		        try {
		          const result = await FetchPreloadPkId(token, 'view_tb_jpercefr_emaqau', formData['TB_JPERCEFR_org.limp.basework.impl.PreloadFakeId']);
		          
			    		setFormData((prevData: any) => ({ ...prevData, ['TB_JPERCEFR_org.limp.basework.impl.PreloadFakeId']: result['TB_JPERCEFR_org.limp.basework.impl.PreloadFakeId'] }));
		        } catch (error) {
		          if (error instanceof Error) {
						const wrapError = error as { response?: { status: number, data: any } };
						if (wrapError.response?.status == 400) {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const validationRules = {
			    "gfaazpnv": () => {
			      const isFileUploaderValid = gfaazpnv_fileUploaderRef.current?.validate();
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_jpercefr_emaqau', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...prevData,
					pkJpercefr: response.data.data.pkJpercefr,
				}));
	...
	const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const newErrors: { [key: string]: string } = {};
				    		...
				    		if (validationRules["gfaazpnv"]) {
						    	const errorMsg = validationRules["gfaazpnv"]();
						
								if (errorMsg != '') {
						    		newErrors["gfaazpnv"] = errorMsg;
						    	}
						    }
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_jpercefr_emaqau" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={groupCardStyle}>
		                            <h2 className={groupTitleStyle}>文件导入</h2>
		                            <p className={groupDescriptionStyle}>
		                                
		                            </p>
		                            <div className={groupContentStyle + ` sm:grid-cols-2`}>
			<input type="hidden" name="xahuwknf" placeholder="" value={formData.xahuwknf || ''} />
			<div className={oneRowStyle + ` sm:col-span-2`}>
			    <label htmlFor="tb_jpercefr_gfaazpnv" className={labelStyle}>
			        账单文件
			    </label>
			    <div className={uploadStyle}>
	                  <FileUploader
		                    ref={gfaazpnv_fileUploaderRef}
		                    id="tb_jpercefr_gfaazpnv"
		                    name="gfaazpnv"
		                    label="DOC, IMAGE, ZIP up to 10MB, up to 5 files"
		                    rtn="TB_JPERCEFR"
		                    rfn="gfaazpnv"
		                    rfpk={formData['TB_JPERCEFR_org.limp.basework.impl.PreloadFakeId']}
		                    cfn={''}
		                    annexFiles={formData['gfaazpnv']}
	                  />
                </div>
			</div>
								    </div>
