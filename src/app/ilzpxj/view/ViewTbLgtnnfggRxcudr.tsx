...
export default function ViewPage<T extends object = { [key: string]: any }>({ readOnly, initialData, onSave, onSubmit, onCancel }: ViewPageProps<T>) {
	...
	const [formData, setFormData] = useState(() => {
    	...
    	const [focusedlistXaglooRowId, setFocusedlistXaglooRowId] = useState<string | null>(null);
		const [editinglistXaglooRowId, setEditinglistXaglooRowId] = useState<string | null>(null);
		const [isConfirmDeletionlistXaglooRowOpen, setIsConfirmDeletionlistXaglooRowOpen] = useState(false);	
    ...
    useEffect(() => {
		...
		if (formData.pkLgtnnfgg !== '') {
			axios.post(bpcApiUrl + '/tableview/queryformdata/view_tb_lgtnnfgg_rxcudr/' + formData.pkLgtnnfgg, {}, {
				headers: {
   ...
   const handleDateError = (itemName: string, dateError: DateValidationError) => {
	...
	const submitFormData = () => {
		...
		setIsLoading(true);
	      ...
	      axios.post(bpcApiUrl + '/tableview/remixjsondata/view_tb_lgtnnfgg_rxcudr', trimObjectValues(formData), {
	        headers: {
	          'grooveToken': token
	        }
	      }).then(response => {
	    	if (response.data && response.data.success) {
	    		setFormData((prevData: any) => ({
					...
					pkLgtnnfgg: response.data.data.pkLgtnnfgg,
				    			pkIytnlwnu: response.data.data.pkIytnlwnu,
				}));
    ...
    return (
        <>
            ...
            <ThemeProvider theme={theme}>
                 <form id="form_view_tb_lgtnnfgg_rxcudr" onSubmit={handleSubmit}>
                    <div className="space-y-12">
      					    	<div className={listFitCardStyle}>	
	      					    	<table className={tableStyle}>
	      					    		...
	      					    		<div className="flex justify-between items-center w-full">
		      					    		...
		      					    		{!!!isViewReadOnly && (
												<Button
													...
													startIcon={<AddIcon />}
													onClick={(e) => {
														setFocusedlistXaglooRowId(null);
														setEditinglistXaglooRowId(null);
	
														const newRow: any = {
	
														};
	
														setFormData((prevFormData: { listXagloo: any; }) => ({
															...prevFormData,
															listXagloo: [
																...prevFormData.listXagloo,
																newRow,
															],
														}));
													}}
	      					    		...
	      					    		<thead>
	      					    			<tr>
	      					    				...
	      					    				<th className={tableThStyle}>当月结算电量</th>
	      					    				<th className={tableThStyle}>当月代购电价</th>
	      					    				<th className={tableThStyle}>当月结算电价含税</th>
	      					    				<th className={tableThStyle}>当月结算电价不含税</th>
	      					    				<th className={tableThStyle}>当月结算电费</th>
	      					    				<th className={tableThStyle}>当月营业收入</th>
	      					    				<th className={tableThStyle}>当月销项税额</th>
	      					    				<th className={tableThStyle}>累计结算电量</th>
	      					    				<th className={tableThStyle}>累计结算电价含税</th>
	      					    				<th className={tableThStyle}>累计结算电价不含税</th>
	      					    				<th className={tableThStyle}>累计结算电费</th>
	      					    				<th className={tableThStyle}>累计营业收入</th>
	      					    				<th className={tableThStyle}>累计销项税额</th>
	      					    				<th className={tableThStyle}>备注</th>
	      					    			 </tr>
	      					    		</thead>
	      					    		<tbody className={tableTbodyStyle}>
											{formData.listXagloo.map((item: any, index: number) => {
												const rowIndex = 'row_' + index;
												
												const isFocused = rowIndex === focusedlistXaglooRowId && !isViewReadOnly;
												const isEditing = rowIndex === editinglistXaglooRowId && !isViewReadOnly;
												
												return (
													<tr 
													key={rowIndex}
													onClick={() => {
														if (!focusedlistXaglooRowId || editinglistXaglooRowId !== rowIndex) {
															setFocusedlistXaglooRowId(rowIndex);
															setEditinglistXaglooRowId(null);
														}
													}}
													className={[
														...
														isFocused ? 'bg-blue-50' : 'hover:bg-gray-50',
			      					    				...
			      					    				{isEditing ? (
															<RowInputRenderer
																...
																value={item.lthpoiid}
																onChange={(newValue) => {
																	const updatedList = formData.listXagloo.map((row: any, idx: number) => {
																	...
																	<ButtonGroup size="small" color="primary" orientation="vertical" sx={{ 
																		...
																		<Button startIcon={<InsertBeforeIcon />}
																			...
																			onClick={(e) => {
																				...
																				const currentList = formData.listXagloo || [];
		
																				const newRow: any = {
																				};
		
																				let insertIndex = currentList.findIndex((row: any, idx: number) => ('row_' + idx) === focusedlistXaglooRowId);
																		...
																		<Button startIcon={<InsertNextIcon />}
																			...
																			onClick={(e) => {
																				...
																				let focusedIndex = currentList.findIndex((row: any, idx: number) => ('row_' + idx) === focusedlistXaglooRowId);
																		...
																		<Button startIcon={<EditIcon />}
																			...
																			onClick={(e) => {
																				...
																				setEditinglistXaglooRowId(rowIndex);
																			}}
																			sx={{
																				justifyContent: 'flex-start'
																			}}
																		...
																		<Button startIcon={<DeleteIcon />}
																			...
																			onClick={(e) => {
																				...
																				setEditinglistXaglooRowId(null);
																				setIsConfirmDeletionlistXaglooRowOpen(true);
																			}}
																			sx={{
																				justifyContent: 'flex-start'
																			}}
	      					    	...
	      					    	<SimpleConfirmDialog
										open={isConfirmDeletionlistXaglooRowOpen}
										onConfirm={() => {
											setIsConfirmDeletionlistXaglooRowOpen(false);
		
											// 1. Find the focused element outside of setFormData.
											const foundElement = formData.listXagloo.find((row: any, idx: number) => {
												const rindex = 'row_' + idx;
		
												return rindex == focusedlistXaglooRowId;
											});
		
											// 2. Use setFormData uniformly for all immutable state updates.
											setFormData((prevData: any) => {
												// 2a. Filter the new data list array (remove the focused row).
												const newList = prevData.listXagloo.filter((row: any, idx: number) => {
													const rindex = 'row_' + idx;
													return rindex !== focusedlistXaglooRowId;
												});
												...
												let discardedDataList = [...(prevData.formDataBin?.listXagloo || [])];
												...
												return {
													...prevData,
													['listXagloo']: newList, // Update data list
													formDataBin: {
														...prevData.formDataBin, // Preserve other possible key-value pairs in formDataBin
														listXagloo: discardedDataList,       // Update the data List array in formDataBin
													},
												};
											});
			...
			<div className={oneColumnStyle}>
	            <label htmlFor="tb_lgtnnfgg_fuljpehn" className={labelStyle}>
	                编制单位
	            </label>
	            <div className="mt-2">
	            {isViewReadOnly ? (
	            	<Typography variant="body2" gutterBottom>{formData.fuljpehn || ''}</Typography>
	            ) : (
	              <FormControl fullWidth ref={(el) => {
													if (el) fieldRefs.current['fuljpehn'] = el;
												}}>
		              <TextField
		                    id="tb_lgtnnfgg_fuljpehn"
		                    name="fuljpehn"
		                    value={formData.fuljpehn || ''}
		                    onChange={handleInputChange}
		                    size="small"
		                    variant="outlined"
		                    error={errors.fuljpehn ? true : false}
		                    helperText={errors.fuljpehn}
		                    slotProps={{
					            input: {
